...
32     format('|',32(i4,'|'))
     ...
     subroutine genspacepart(GridEdge,GridVertex)
       ...
       !  s2 = s1+1 
       !
       ! First region gets nelemd+1 elements per partition
       ! Second region gets nelemd elements per partition
       ...
       do k=1,nelem
          ...
          if (id<=s1) then
             tmp1 = id/(nelemd+1)
             GridVertex(k)%partition = tmp1+1
          else
             id=id-s1
             tmp1 = id/nelemd
             GridVertex(k)%partition = extra + tmp1+1
          endif
       enddo
#if 0
       write(iulog,*)'Space-Filling Curve Parititioning: '
       do k=1,nelem
          write(iulog,*) k,GridVertex(k)%partition
       enddo
