       ...
       do c = begchunk,endchunk
          ...
          if ( i<=N_MMRS ) then
             ...
             case ("molec/cm3","/cm3","molecules/cm3","cm^-3","cm**-3")
                to_mmr(:ncol,:) = (molmass(i)*1.e6_r8*boltz*state(c)%t(:ncol,:))/(amass*state(c)%pmiddry(:ncol,:))
             case ('kg/kg','mmr')
                to_mmr(:ncol,:) = 1._r8
             case ('mol/mol','mole/mole','vmr','fraction')
                to_mmr(:ncol,:) = molmass(i)/amass
             case default
             ...
             call outfld( 'WFRC_'//trim(fields(i)%fldnam), fields(i)%data(:ncol,:,c+fields(i)%chnk_offset), ncol, state(c)%lchnk )
