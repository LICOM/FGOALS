...
!---------------------------------------------------------------------
  ...
  use pmgrid,         only: plon, plev, plevp, beglatxy, endlatxy, beglonxy, endlonxy
...
! Get qbo file
!---------------------------------------------------------------------
       call wrap_open( qbo_forcing_file, NF_NOERR, ncid )
...
!---------------------------------------------------------------------
       if (has_monthly_data) then
          ...
          call wrap_get_var_realx( ncid, uqboid, u_inp )
          ...
          call wrap_get_var_realx( ncid, cosqboid,  cosq_inp )
          call wrap_get_var_realx( ncid, sinqboid,  sinq_inp )
...
!---------------------------------------------------------------------
       do ktop = 1,pver
          if( hypm(ktop) >= p_inp(1) ) then
             exit
          end if
       end do
       write(iulog,*) 'qbo_init: ktop = ', ktop, hypm(ktop)/hPa2Pa

!---------------------------------------------------------------------
! Find last model layer within qbo range
!---------------------------------------------------------------------
       do kbot = pver,ktop,-1
          if( hypm(kbot) <= p_inp(levsiz) ) then
             exit
          end if
       end do
       write(iulog,*) 'qbo_init: kbot = ', kbot, hypm(kbot)/hPa2Pa
...
!---------------------------------------------------------------------
        do k = ktop,kbot
           do kk = 1,levsiz-1
              if( p_inp(kk+1) > hypm(k) ) then
                 exit
              end if
           end do
           fac1 = (hypm(k) - p_inp(kk+1)) / (p_inp(kk) - p_inp(kk+1))
           fac2 = (p_inp(kk) - hypm(k)  ) / (p_inp(kk) - p_inp(kk+1))
           if (has_monthly_data) then
...
!================================================================================================
    ...
    subroutine qbo_relax( state, ptend, uzm )
!------------------------------------------------------------------------
...
!--------------------------------------------------------------------------------
    ...
    real(r8), intent(in)               :: uzm(pcols,pver)      ! zonal mean wind
