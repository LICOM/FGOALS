...
! Atmosphere/clouds - cldprmc [mcica]
      real(kind=r8) :: cldfmc(ngptsw,nlay+1)    ! cloud fraction [mcica]
      real(kind=r8) :: ciwpmc(ngptsw,nlay+1)    ! cloud ice water path [mcica]
      real(kind=r8) :: clwpmc(ngptsw,nlay+1)    ! cloud liquid water path [mcica]
      real(kind=r8) :: relqmc(nlay+1)           ! liquid particle size (microns)
      real(kind=r8) :: reicmc(nlay+1)           ! ice particle effective radius (microns)
      real(kind=r8) :: dgesmc(nlay+1)           ! ice particle generalized effective size (microns)
      real(kind=r8) :: taucmc(ngptsw,nlay+1)    ! cloud optical depth [mcica]
      real(kind=r8) :: taormc(ngptsw,nlay+1)    ! unscaled cloud optical depth [mcica]
      real(kind=r8) :: ssacmc(ngptsw,nlay+1)    ! cloud single scattering albedo [mcica]
      real(kind=r8) :: asmcmc(ngptsw,nlay+1)    ! cloud asymmetry parameter [mcica]
      real(kind=r8) :: fsfcmc(ngptsw,nlay+1)    ! cloud forward scattering fraction [mcica]
...
! Atmosphere/clouds/aerosol - spcvrt,spcvmc
      ...
      real(kind=r8) :: zcldfmc(nlay+1,ngptsw)   ! cloud fraction [mcica]
      real(kind=r8) :: ztaucmc(nlay+1,ngptsw)   ! cloud optical depth [mcica]
      real(kind=r8) :: ztaormc(nlay+1,ngptsw)   ! unscaled cloud optical depth [mcica]
      real(kind=r8) :: zasycmc(nlay+1,ngptsw)   ! cloud asymmetry parameter [mcica] 
      real(kind=r8) :: zomgcmc(nlay+1,ngptsw)   ! cloud single scattering albedo [mcica]
...
! convert effective radius to generalized effective size using method of Mitchell, JAS, 2002:
         ...
         do l = 1, nlayers-1
            do ig = 1, ngptsw
               cldfmc(ig,l) = cldfmcl(ig,iplon,nlayers-l)
               taucmc(ig,l) = taucmcl(ig,iplon,nlayers-l)
               ssacmc(ig,l) = ssacmcl(ig,iplon,nlayers-l)
               asmcmc(ig,l) = asmcmcl(ig,iplon,nlayers-l)
               fsfcmc(ig,l) = fsfcmcl(ig,iplon,nlayers-l)
               ciwpmc(ig,l) = ciwpmcl(ig,iplon,nlayers-l)
               clwpmc(ig,l) = clwpmcl(ig,iplon,nlayers-l)
            enddo
            reicmc(l) = reicmcl(iplon,nlayers-l)
            if (iceflag .eq. 3) then
               dgesmc(l) = 1.5396_r8 * reicmcl(iplon,nlayers-l)
            endif
            relqmc(l) = relqmcl(iplon,nlayers-l)
         enddo

! If an extra layer is being used in RRTMG, set all cloud properties to zero in the extra layer.

         cldfmc(:,nlayers) = 0.0_r8
         taucmc(:,nlayers) = 0.0_r8
         ssacmc(:,nlayers) = 1.0_r8
         asmcmc(:,nlayers) = 0.0_r8
         fsfcmc(:,nlayers) = 0.0_r8
         ciwpmc(:,nlayers) = 0.0_r8
         clwpmc(:,nlayers) = 0.0_r8
         reicmc(nlayers) = 0.0_r8
         dgesmc(nlayers) = 0.0_r8
         relqmc(nlayers) = 0.0_r8
         taua(nlayers,:) = 0.0_r8
         ssaa(nlayers,:) = 1.0_r8
         asma(nlayers,:) = 0.0_r8
