...
subroutine oldcloud_init()
   ...
   dei_idx    = pbuf_get_fld_idx('DEI')
   mu_idx     = pbuf_get_fld_idx('MU')
   lambda_idx = pbuf_get_fld_idx('LAMBDAC')
...
real(r8) :: wavmax(nswbands) = &  ! Max wavelength (micro-meters) of interval
   ...
   if(oldicewp) then
     ...
     cicewp(1:pcols,1:pver) =  1000.0_r8*pbuf(iciwp_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
   endif
