...
subroutine streams_exp_set( streams_exp, datasource, filePath, fldListfile,      &
 ...
 character(*), intent(IN), optional :: alogo
 ...
 if ( present(alogo)        ) streams_exp%alogo        = alogo
...
subroutine streams_exp_write_strm_txt( stream_filename, streams_exp )
 ...
 character(*), parameter :: sub = "write_streams_txt"
   ...
   write(unit,*) "  </variableNames>"
   if ( trim(streams_exp%alogo) /= "linear" )then
      write(unit,*) "  <tInterpAlgo>"
      write(unit,*) "     ", trim(streams_exp%alogo)
      write(unit,*) "  </tInterpAlgo>"
   end if
   write(unit,*) "  <filePath>"
   write(unit,*) "     ", trim(streams_exp%FilePath)
   write(unit,*) "  </filePath>"
   write(unit,*) "  <fileNames>"
...
logical function is_streams_expected( stream, streams_exp )
   ...
   call shr_stream_getTInterpAlgo(    stream, alogo        )
   ...
   if ( trim(alogo) /= trim(streams_exp%alogo) ) &
          is_streams_expected = .false.
   if ( .not. is_streams_expected ) write(*,*) "alogo different"
   n = 1
...
program test_shr_streams
  ...
  character(SHR_KIND_CS) :: clmncep(12) = (/ &
                                            "clmncep.T62.1948-01.nc", &
                                            "clmncep.T62.1948-02.nc", &
                                            "clmncep.T62.1948-03.nc", &
                                            "clmncep.T62.1948-04.nc", &
                                            "clmncep.T62.1948-05.nc", &
                                            "clmncep.T62.1948-06.nc", &
                                            "clmncep.T62.1948-07.nc", &
                                            "clmncep.T62.1948-08.nc", &
                                            "clmncep.T62.1948-09.nc", &
                                            "clmncep.T62.1948-10.nc", &
                                            "clmncep.T62.1948-11.nc", &
                                            "clmncep.T62.1948-12.nc"  &
                                          /)
  ...
  do series = 1, num_series
     ...
     if (      series  == bogus_TEST )then
        ...
        call streams_exp_set( stream_exp, datasource="CLMNCEP", &
                              ...
                              domfilepath="/fs/cgd/csm/inputdata/atm/datm7/",        &
                              domfilename="domain.T62.050609.nc", &
                              nfiles=12, filenames=clmncep(1:12), alogo="lower" )
        yearAlign = 1
        yearFirst = 1948
        yearLast  = 1948
#ifndef LINUX
  ...
  call shr_stream_findBounds(streams(1),mDateIn,        secIn,      &
                           &   mDateLB,dDateLB,secLB,n_lb,fileLB,   &
                           &   mDateUB,dDateUB,secUB,n_ub,fileUB, rCode )
